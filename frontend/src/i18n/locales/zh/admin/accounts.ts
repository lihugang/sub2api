export default {
    accounts: {
      ...
      openai: {
        ...
        longContextBillingDesc: '默认开启。仅当该账号的上游不会按模型阈值收取 OpenAI API 长上下文费率时关闭。',
      ...
      anthropic: {
        ...
        mockCacheDesc:
          '按 Claude 请求最后 4 个 ephemeral 缓存检查点计算用量。重复前缀记为缓存读取，新前缀或连续 5 分钟无请求的前缀记为缓存写入；每次请求另有 2% 概率整次不命中。响应、计费和用量日志保持一致。'
      },
