...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldTimeRateRules, group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelAllowlist, group.FieldCodexModelsManifestConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
	...
	for i := range columns {
		...
		case group.FieldTimeRateRules:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field time_rate_rules", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.TimeRateRules); err != nil {
					return fmt.Errorf("unmarshal field time_rate_rules: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("time_rate_rules=")
	builder.WriteString(fmt.Sprintf("%v", _m.TimeRateRules))
	builder.WriteString(", ")
