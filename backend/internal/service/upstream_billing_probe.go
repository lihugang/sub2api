...
// accounts. The lowercase "free" marker keeps the long-standing opt-out
// behavior for free accounts.
...
// applyUpstreamBillingProbeAccountRates derives the account priority from a
// successful probe and persists it with the snapshot (rate write-back is
// governed separately by the upstream billing rate sync switch).
