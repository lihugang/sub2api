...
const apiKeyAuthSnapshotVersion = 24 // v24: upstream group model_allowlist field (renamed from models_list_config, enforcing semantics) plus free Fast / reasoning-effort ceiling / Codex manifest snapshot fields; also forces refresh of local pre-v20 snapshots (user routing notice, group profit control, search/audio/video_model_prices billing fields)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		User: APIKeyAuthUserSnapshot{
			...
			ModelRoutingNoticeMode:     NormalizeModelRoutingNoticeMode(apiKey.User.ModelRoutingNoticeMode),
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			TimeRateRules:                   append([]TimeRateRule(nil), apiKey.Group.TimeRateRules...),
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		User: &User{
			...
			ModelRoutingNoticeMode:     NormalizeModelRoutingNoticeMode(snapshot.User.ModelRoutingNoticeMode),
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			TimeRateRules:                   append([]TimeRateRule(nil), snapshot.Group.TimeRateRules...),
